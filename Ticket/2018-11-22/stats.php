...
*/
	...
	//Por tiempo
	$registros=array();
	$sql = "SELECT idreclamo, t.nombre AS tecnico, nodo, problema, numcliente, time FROM reclamo r INNER JOIN tecnico AS t ON r.tecnico = t.idtecnico WHERE `time` > DATE_SUB(NOW(), INTERVAL '3:0:".$intervalo."' HOUR_SECOND) ORDER BY ".$orden.";";
	if($intervalo == -1){
		$sql = "SELECT idreclamo, t.nombre AS tecnico, nodo, problema, numcliente, time FROM reclamo r INNER JOIN tecnico AS t ON r.tecnico = t.idtecnico ORDER BY ".$orden.";";
	}
...
*/	
	...
	//Por Cliente
	$sql = "SELECT numcliente AS cliente, COUNT(*) AS cant FROM reclamo r WHERE `time` > DATE_SUB(NOW(), INTERVAL '3:0:".$intervalo."' HOUR_SECOND) GROUP BY numcliente ORDER BY cant DESC LIMIT 15;";
	if($intervalo == -1){
		$sql = "SELECT numcliente AS cliente, COUNT(*) AS cant FROM reclamo r GROUP BY numcliente ORDER BY cant DESC LIMIT 15;";
	}
...
<body bgcolor="B7D590">
	...
	<table id="result" class="egt" align="center"  border="1" bgcolor="FFFFFF">
		<tr bgcolor="CFCFCF">
			<th width="150"><a class="tes" href='stats.php?orden=<?php if($orden=="tecnico ASC"){echo "tecnico DESC";}else{echo "tecnico ASC";};?>&intervalo=<?php echo $intervalo;?>'>
				<p class="pointer">Tecnico
					<?php if ($orden == "tecnico ASC" ) echo '&#9650;' ; ?>
					<?php if ($orden == "tecnico DESC" ) echo '&#9660;' ; ?>
				</a></p>
			</a></th>
			<th width="150"><a class="tes" href='stats.php?orden=<?php if($orden=="nodo ASC"){echo "nodo DESC";}else{echo "nodo ASC";};?>&intervalo=<?php echo $intervalo;?>'>
				<p class="pointer">Panel
					<?php if ($orden == "nodo ASC" ) echo '&#9650;' ; ?>
					<?php if ($orden == "nodo DESC" ) echo '&#9660;' ; ?>
				</a></p>
			</a></th>
			<th width="200"><a class="tes" href='stats.php?orden=<?php if($orden=="problema ASC"){echo "problema DESC";}else{echo "problema ASC";};?>&intervalo=<?php echo $intervalo;?>'>
				<p class="pointer">Reclamo
					<?php if ($orden == "problema ASC" ) echo '&#9650;' ; ?>
					<?php if ($orden == "problema DESC" ) echo '&#9660;' ; ?>
				</a></p>
			</a></th>
			<th width="90"><a class="tes" href='stats.php?orden=<?php if($orden=="numcliente ASC"){echo "numcliente DESC";}else{echo "numcliente ASC";};?>&intervalo=<?php echo $intervalo;?>'>
				<p class="pointer">Cliente
					<?php if ($orden == "numcliente ASC" ) echo '&#9650;' ; ?>
					<?php if ($orden == "numcliente DESC" ) echo '&#9660;' ; ?>
				</a></p>
			</a></th>
			<th width="170"><a class="tes" href='stats.php?orden=<?php if($orden=="time ASC"){echo "time DESC";}else{echo "time ASC";};?>&intervalo=<?php echo $intervalo;?>'>
...
<?php
	if($pagina > 0 ){
	?>
		<th align="left"><a href='stats.php?intervalo=<?php echo $intervalo;?>&pagina=<?php echo $pagina-1;?>&orden=<?php echo $orden;?>'><b><i><< Anterior</i></b></a></th>
<?php
	}
	?>
		<td></td>
<?php
	...
	?>
		<th align="right"><a href='stats.php?intervalo=<?php echo $intervalo?>&pagina=<?php echo $pagina+1;?>&orden=<?php echo $orden;?>'><b><i>Siguiente >></i></b></a></th>
<?php
	}
	?>
