...
// ?cl=321321&ip=172.168.18.66
	...
	$servername_panel = "172.16.2.173";
    $database_panel = "SS-DB";
    $username_panel = "ross";
    $password_panel = "ReadOnly@SS";

    $conn_panel = new mysqli($servername_panel, $username_panel, $password_panel, $database_panel);
    if ($conn_panel->connect_error) {
        die("Connection failed: " . $conn_panel->connect_error);
    }
	
	/* -------DEPRECATED
    $octetos = explode('.', $ipcliente);
    $ip_panel = $octetos[0] . '.' . $octetos[1] . '.' . $octetos[2].'.';
    $sql_panel = "SELECT * FROM panel where panel.ip like '%" . $ip_panel . "%'";
    $result_panel = $conn_panel->query($sql_panel);
    */
...
<?php
		...
		/* --------DEPRECATED
		if ($result_panel->num_rows > 0) {
			while ($row = $result_panel->fetch_assoc()) {
				$ip_panel1 = $row["ip"];
				$ip_antena = $ipcliente;
				if ($row["marca"] == 'Cambium')
		...
		*/
		...
		$sql = "SELECT nombre, marca, ip, mask FROM panel;";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
...
?>
                <table id="data_client" class="egt" align="center"  border="1" bgcolor="FFFFFF">
                    <tr bgcolor="CFCFCF">
                        <th width="150">
                            <p class="pointer">Cliente</p>
                        </th>
                        <th width="150">
                            <p class="pointer">Router</p>
                        </th>
                        <th width="155">
                            <p class="pointer">Antena</p>
                            </a></th>
                        <th width="150">
                            <p class="pointer">Panel</p>
                            </a></th>
                        <th width="155">
                            <p class="pointer">Panel IP</p>
                            </a></th>
                    </tr>
                    <tr>
                        <th width='150' align='left'>
                        ...
                        <th width='150'  align='left'>
                            <a href="http://<?php echo $ipcliente; ?>" target="_blank"><?php echo $ipcliente; ?></a>
                        </th>
                        <th width='155'  align='left'>
                            <a href="http://<?php echo $ip_antena; ?>:6969" target="_blank"><?php echo $ip_antena; ?></a>
                        </th>
                        <th width='150'  align='left'>
                            <?php echo $nombre_panel; ?>
                        </th>
                        <th width='155'  align='right'>
                            <a href="http://<?php echo $ip_panel1; ?>:6969" target="_blank"><?php echo $ip_panel1; ?></a>
                        </th>
